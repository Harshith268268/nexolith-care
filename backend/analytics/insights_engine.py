...
class AIInsightsEngine:
    ...
    Guarantees 100% metadata isolation.
    """
    ...
    def analyze_member_health(self, member_id) -> dict:
        ...
        if not reports:
            return {
                ...
                "risk_score": "No data",
                "overall_status": "No medical report data available yet.",
                "latest_warnings": [],
                "insights": [
        ...
        focus_params = ['Glucose', 'Fasting Glucose', 'HbA1c', 'Cholesterol', 'LDL', 'HDL', 'Hemoglobin', 'Blood Pressure', 'Systolic BP', 'Vitamin D', 'Vitamin B12', 'Platelets', 'PCV', 'MCV', 'MCH', 'MCHC', 'RBC Count', 'WBC Count']

        for param, history in parameter_history.items():
            if extractor.is_metadata_term(param):
                continue
            ...
            if len(history) >= 2:
                ...
                if pct_change != 0:
                    ...
                    elif trend_type == "decreased" and any(x in param.lower() for x in ['hemoglobin', 'vitamin', 'hdl', 'rbc']):
                        insights.append(f"{param} {trend_type} by {abs_pct}% compared to previous report (currently {latest['value']} {latest['unit']}).")
                    else:
        ...
        for param, history in parameter_history.items():
            ...
            if latest['status'] in ['Borderline', 'Critical']:
                ...
                elif 'hemoglobin' in param.lower() or 'rbc' in param.lower():
        ...
        latest_warns = [f"Abnormal parameter detected: {k}" for k, v in parameter_history.items() if v[-1]['status'] in ['Borderline', 'Critical'] and not extractor.is_metadata_term(k)]

        return {
            "abnormal_parameter_count": total_abnormal_count,
            "risk_score": risk_score,
            "overall_status": overall_status,
            "latest_warnings": list(set(warnings + latest_warns))[:5],
            "insights": insights[:5],
